<?php 
  ...
  //}
?>
<?php $reminders = get_task_reminders($project_id, $item_id, $current_role);?>
<a class="add_reminder_btn<?php echo $item_id;?> btn btn-rose btn-round" data-toggle="modal" data-target="#addReminderModal<?php echo $item_id;?>">Notifications <span class="badge count<?php echo $item_id;?>"><?php echo count($reminders);?></span>
<b class="caret"></b></a>
                                        <!-- Reminders modal -->
                                            <div class="modal fade" id="addReminderModal<?php echo $item_id;?>" tabindex="-1" role="dialog" aria-labelledby="myReminderModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-notice">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="material-icons">clear</i></button>
<h5 class="modal-title" id="myReminderModalLabel"><?php echo get_task_name($task_id);?> Notifications</h5>                                                        </div>
                                ...
                                <li class="timeline-inverted">
                                    ...
                                    <div class="timeline-panel">
                                        <?php if($current_role==1 || $current_role==2){ ?><a rowno="<?php echo $item_id;?>" id="<?php echo $reminder['id'];?>" class="remove_reminder btn btn-simple btn-danger btn-icon pull-right"><i class="material-icons">close</i></a><?php } ?>
                                        ...
                                        <div class="togglebutton pull-right">
                                                <label class="text-default">
                                                  <input id="privacy_settings<?php echo $reminder['id'];?>" name="privacy_settings" type="checkbox" <?php if($reminder['privacy_settings']==0){ ?> checked <?php } ?> onclick="set_privacy(<?php echo $reminder['id'];?>, <?php echo $item_id;?>, 'Reminder');" >Private
                                                </label>
                                        </div>
                                        <?php } ?>
                                    </div>
...
<div class="card">
                                ...
                                <div class="card-content">
                                    ...
                                    <div class="toolbar">
<form id="RemindersForm<?php echo $item_id;?>" method="post">
...
<input type="hidden" id="reminder_item_id" name="reminder_item_id" value="<?php echo $item_id;?>">
																<input type="hidden" id="reminder_project_id" name="reminder_project_id" value="<?php echo $project_id;?>">
                                                        ...
                                                        <?php 
$reminder_users = get_task_reminder_users($task_id);
...
<div class="form-group label-floating"><label>Date Task Scheduled</label>
																	<input class="form-control" type="text" name="date" id="date" required="true" value="<?php echo date("d/m/Y", strtotime($reminder_date));?>" placeholder="Date" readonly/>
																</div>		
<?php $reminders = get_task_reminder_message($task_id);?>
<div class="form-group label-floating">																	<select rowno="<?php echo $item_id;?>" id="message_id" name="message_id" required="true" class="selectpicker" data-style="select-with-transition" title="Reminder Type">
...
if(count($reminders)>0){
                                                        ...
                                                        <?php } } ?>
																	...
																	<textarea class="form-control message_<?php echo $item_id;?>" name="message" id="message" required="true" placeholder="Message"/></textarea>
																</div>
																<div class="form-footer text-right">
																	<button rowno="<?php echo $item_id;?>" type="button" class="btn btn-rose btn-fill add_new_reminder">Add</button>
																</div>
