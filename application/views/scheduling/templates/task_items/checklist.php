...
<?php $checklists = get_template_task_checklist($item['template_id'], $item['id']);?>
<?php 
if($item['checklist']!=""){$task_checklist = explode(",", $item['checklist']);}else{ $task_checklist = array();} ?>
<a class="btn btn-primary btn-round add_checklist_btn<?php echo $item['id'];?>" data-toggle="modal" data-target="#addChecklistModal<?php echo $item['id'];?>">Checklist <span class="badge count<?php echo $item['id'];?> checklist_count<?php echo $item['id'];?>"><?php if(count($checklists) -count($task_checklist)<0){ echo "0"; } else{ echo count($checklists) -count($task_checklist); }?></span>
<b class="caret"></b></a>
                                        <!-- Checklist modal -->
