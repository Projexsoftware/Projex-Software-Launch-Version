...
class Mod_common extends CI_Model {
    ...
    function get_all_records($table = "", $fields = "*",$llimit=0,$ulimit=0,$where = array()) {
...
//          return $this->doOutputList($tree);
    ...
    public function get_all_dashboard_projects(){
      $query = $this->db->query("SELECT * FROM wiz_projects WHERE id IN (SELECT project_id FROM wiz_project_team WHERE team_id = '".$this->session->userdata("user_id")."' AND (team_role=1 OR (team_role!=1 AND is_invitation_send=2))) ORDER BY id DESC LIMIT 6");
      return $query->result_array($query);
    }
