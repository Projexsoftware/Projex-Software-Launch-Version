...
class Supplier_invoices extends CI_Controller {
    ...
    public function populate_project_part_costing() {
        ...
        $data['selected_costing_id'] = $this->input->post('project_id');
        ...
        if(count($pc_ids) > 0){
            $prject_id=$this->mod_project->get_project_id_from_costing_id($this->input->post('project_id'));
        ...
        if ($this->form_validation->run() == FALSE)
    	{
            ...
            if ($invoice_new) {
                ...
                if (isset($_POST['project_cost_partpc'])) {
                    ...
                    foreach ($project_cost_partpc as $key => $value) {
                        ...
                        $part = array(
                            ...
                            'useradditionalcost' => $pcinvoicecostdiff[$key],
                ...
                if (isset($_POST['nicomponent'])) {
                        ...
                        if(($this->input->post('createvariation'))== 1 &&  ($this->input->post('supplierinvoicestatus')=='Approved' OR $this->input->post('supplierinvoicestatus')=='Pending')){
                            ...
                            if ($vId > 0 && $niallowance[$key]==0) {
                                ...
                                $part = array(
                                    ...
                                    'useradditionalcost' => $nilinttotal[$key],
                                    'quantity_type' => 'manual',
                                    'formulaqty' => '',
                                    'formulatext' => '',
                                    'margin' => 0,
                    ...
                    $is_variation_part_without_allowance = $this->mod_common->select_single_records("project_variation_parts", array("variation_id" => $vId));
                    if(count($is_variation_part_without_allowance)==0){
                        $this->mod_common->delete_record("project_variations", array("id" => $vId));
                    }
                }
    		            ...
    		            $supplier_info = $this->mod_common->select_single_records('project_suppliers', $where);
                           ...
                           $new_invoice = array( "Invoices" => array(
                                    			array(
                                    				...
                                    				"Status" => "AUTHORISED",
                                    				"InvoiceNumber" => $_POST['supplierrefrence']."-".$project_info["project_title"], 
                                    				"Reference" => $_POST['supplierrefrence']."-".$project_info["project_title"],
		            ...
		            $supplier_info = $this->mod_common->select_single_records('project_suppliers', $where);
                       ...
                       $new_invoice = array( "Invoices" => array(
                                			array(
                                				...
                                				"DueDate" => $invoice_due_date,
                                				"Status" => "AUTHORISED",
                                				"InvoiceNumber" => 	$invoice_info["supplierrefrence"]."-".$project_info["project_title"], 
                                				"Reference" => 	$invoice_info["supplierrefrence"]."-".$project_info["project_title"],
                                				"LineAmountTypes" => "Exclusive",
    ...
    public function repopulate_all_available_components() {
        ...
        $data['selected_supplier_id'] = join(', ', $this->input->post('supplier_id'));
        $cwhere = "costing_id = ".$data['selected_costing_id']." AND costing_supplier IN (".$data['selected_supplier_id'].") AND costing_type!='sivar' AND client_allowance = 0 AND costing_part_id NOT IN (SELECT costing_part_id FROM project_supplier_invoices_items WHERE supplier_invoice_id = $supplier_invoice_id)";
        }
        else if($this->input->post('stage_id')!="" && $this->input->post('supplier_id')==""){
        $data['selected_stage_id'] = join(', ', $this->input->post('stage_id'));
        $cwhere = "costing_id = ".$data['selected_costing_id']." AND stage_id IN (".$data['selected_stage_id'].") AND costing_type!='sivar' AND client_allowance = 0 AND costing_part_id NOT IN (SELECT costing_part_id FROM project_supplier_invoices_items WHERE supplier_invoice_id = $supplier_invoice_id)";
        }
        else if($this->input->post('supplier_id')!="" && $this->input->post('stage_id')!=""){
        $data['selected_stage_id'] = join(', ', $this->input->post('stage_id'));
        $data['selected_supplier_id'] = join(', ', $this->input->post('supplier_id'));
        $cwhere = "costing_id = ".$data['selected_costing_id']." AND stage_id IN (".$data['selected_stage_id'].") AND costing_supplier IN (".$data['selected_supplier_id'].") AND costing_type!='sivar' AND client_allowance = 0 AND costing_part_id NOT IN (SELECT costing_part_id FROM project_supplier_invoices_items WHERE supplier_invoice_id = $supplier_invoice_id)";
        }
        else{
            $data['selected_supplier_id'] = "";
            $cwhere = "costing_id = ".$data['selected_costing_id']." AND costing_type!='sivar' AND client_allowance = 0 AND costing_part_id NOT IN (SELECT costing_part_id FROM project_supplier_invoices_items WHERE supplier_invoice_id = $supplier_invoice_id)"; 
        }
        ...
        if(count($pc_ids) > 0){
            ...
            $data['component'] = $this->mod_common->get_all_records('project_components', "*", 0, 0, $where, "component_id");
            ...
            $prject_id=$this->mod_project->get_project_id_from_costing_id($this->input->post('costingId'));
            $data['projectname'] = $this->mod_project->get_project_by_name($prject_id['project_id']);
            ...
            foreach ($pc_ids as $kepc => $pc_id) {
                ...
                if ($pc_id == 0) {
                    ...
                    $data['costing_part_details']->var_number = $variation[0]['var_number'];
                     ...
                     $data['costing_part_details']->var_number = 0;   
                    }
                    if(count($supplier_invoiceqi)==0){
                        $supplier_invoice_amount = 0;
                    }
                    else{
                       $supplier_invoice_amount = $supplier_invoiceqi[0]['invoiced_amount'];
                    }
                    $data['costing_part_details']->invoicedquantity = $supplier_invoiceq;
                    $data['costing_part_details']->uninvoicedquantity = $data['costing_part_details']->costing_quantity-$supplier_invoiceq;
                    $data['costing_part_details']->uninvoicebudget= ($data['costing_part_details']->costing_quantity)*($data['costing_part_details']->costing_uc)*(100+$data['costing_part_details']->margin)/100 - $supplier_invoice_amount;
                    if($data['costing_part_details']->uninvoicedquantity<0)
                        $data['costing_part_details']->uninvoicedquantity = 0;
                    if($data['costing_part_details']->uninvoicebudget<0)
                    $data['costing_part_details']->uninvoicebudget = 0;
    ...
    public function updateinvoice($invoice_id) {
        ...
        if ($invoice_new) {
            ...
            if (isset($_POST['nicomponent'])) {
                ...
                $is_variation_part_without_allowance = $this->mod_common->select_single_records("project_variation_parts", array("variation_id" => $vId));
                    if(count($is_variation_part_without_allowance)==0 || (count($nimanualqty)>0 && count($niallowance)>0 && (count($nimanualqty)==count($niallowance)))){
                        $this->mod_common->delete_record("project_variations", array("id" => $vId));
                        $this->mod_common->delete_record("project_variation_parts", array("variation_id" => $vId));
                    }
            }
            ...
            foreach ($existingsipartarr as $key => $value) {
                ...
                $this->mod_common->delete_record('project_supplier_invoices_items', array('id'=>$value));
                   }            
            //Xero Implementation                     
                    if($this->input->post('supplierinvoicestatus')=='Approved'){
