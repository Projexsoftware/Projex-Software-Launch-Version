...
class Tasks extends CI_Controller {
	...
	public function index()
	{
        ...
        $data['tasks'] = $this->mod_common->get_all_records("project_buildz_tasks","*", 0, 0, array("company_id" =>  $this->session->userdata("company_id")));
        $this->stencil->title('Buildz Tasks');
	    $this->stencil->paint('supplierz_buildz/tasks/manage_tasks', $data);
	}

	public function add_task() {
        $this->mod_common->is_company_page_accessible(163);
        $this->stencil->title('Add Buildz Task');
        $this->stencil->paint('supplierz_buildz/tasks/add_new_task');
    }
	 ...
	 public function add_new_task_process() {
	    ...
	    if ($this->form_validation->run() == FALSE)
			...
			if ($add_new_task) {
				...
				redirect(SURL . 'supplierz_buildz/tasks');
			} else {
	...
	if($task_id=="" || !(is_numeric($task_id))){
        ...
        if (count($data['task_edit']) == 0) {
            ...
            $this->stencil->title("Edit Buildz Task");

            $this->stencil->paint('supplierz_buildz/tasks/edit_task', $data);
        }
    }
    ...
    public function edit_task_process() {
        ...
        if($this->input->post('name') != $original_value) {
            $is_unique =  '|is_unique[project_buildz_tasks.name]';
        } else {
            $is_unique =  '';
        }
		...
		if ($this->form_validation->run() == FALSE)
		{
		   ...
		   $this->stencil->title("Edit Buildz Task");
			...
			if ($update_task) {
				...
				redirect(SURL . 'supplierz_buildz/tasks/edit_task/' . $task_id);
			}
		}
    }
