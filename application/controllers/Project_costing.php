                        ...
                        if (count($existing_component) == 0) {
                            ...
                            $part_component_id = $existing_component["component_id"];
                                       ...
                                       $ip_address = $_SERVER['REMOTE_ADDR'];
                                        ...
                                        $is_item_exists =  $this->mod_common->select_single_records("project_scheduling_items", array("task_id" => $new_task_id, "parent_item_id" => $costing_part_idd, "stage_id"=>$gstage_id[$i], "project_id" => $schedule_project_info['id']));
                                ...
                                $is_default_checklist_exists =  $this->mod_common->select_single_records("project_scheduling_item_checklists", array("name" => "In Progress", "item_id" => $item_id, "project_id" => $schedule_project_info['id']));
                                
                                if(count($is_default_checklist_exists)==0){
                                    $checklist_array = array(
                                ...
                                $is_default_checklist_exists =  $this->mod_common->select_single_records("project_scheduling_item_checklists", array("name" => "Complete", "item_id" => $item_id, "project_id" => $schedule_project_info['id']));
                                if(count($is_default_checklist_exists)==0){
	...
	public function update_project_costing_process($costing){
        ...
        $selected_tab = $this->input->post('selected_tab');
            ...
            if ($costing > 0) {
             ...
             $gstage_id = $this->input->post('stage_id');
             /*echo "<pre>";
             print_r($this->input->post('stage_id'));
             echo "Stages = ".count($this->input->post('stage_id'));
             echo "Components = ".count( $component_id);
             echo "Quantity = ".count( $quantity);
             echo "UC = ".count($component_uc);
             echo "Margin = ".count($margin);
             echo "Component UOM = ".count($component_uom);*/
    ...
    public function export_pdf($type, $id){
        ...
        if ($id > 0 && $type!="") {
            ...
            if($type=="allowances"){
                ...
                foreach($data["stages"] as $key => $stage){
                    $data["stages"][$key]["costing_parts"] = $this->mod_project->get_costing_parts_by_project_id($id, "specification", $stage["stage_id"]);
                }
                 $html = $this->load->view('project_costing/specifications_pdf', $data, true); 
            }
            else{
         ...
         foreach ($data['prjprts'] as $key => $prjprt) {
          ...
          foreach ($data['components'] as $component) {
            if ($prjprt->component_id == $component["component_id"]) {
              ...
              $my_data[$key]['component_name'] = escapeString($component["component_name"]);
  ...
  foreach ($prjParts as $key => $prjprt) {
   ...
   public function add_new_client(){
	...
	<td><input type="hidden" name="stage_id[]" value="'.$data[0].'">'.$data[0].'</td>
                                                           ...
                                                           <input rno="'.$count.'" class="form-control sub-category-form-control" type="text" name="sub_category[]" id="subcategoryfield'.$count.'" value=""/>
                                                           </td><td><input type="hidden" name="part_name[]" value="'.$data[1].'">'.$data[1].'</td>
                                                           <td><input type="hidden" name="component_id[]" value="'.$data[2].'">'.$data[2].'<input type="hidden" name="component_description[]" id="componentDescription'.$count.'" rno ="'.$count.'" value="'.$data[3].'"></td>
                                                           <td class="component_description">'.$data[3].'</td>
                                                           <td><input type="hidden" name="supplier_id[]" value="'.$data[4].'">'.$data[4].'<input type="hidden" class="form-control" name="quantity_type[]" id="quantity_type'.$count.'" rno ="'.$count.'" value="manual"></td>
                                                           <td><input rno="'.$count.'" type="hidden" class="qty" id="manualqty'.$count.'" name="manualqty[]" value="'.$data[5].'">'.$data[5].'</td>
                                                           <td><input type="hidden" name="component_uom[]" value="'.$data[6].'">'.$data[6].'</td>
                                                           <td><input type="hidden" name="component_uc[]" id="ucostfield'.$count.'" value="'.$data[7].'">'.$data[7].' <input type="hidden" id="order_unit_cost'.$count.'" rno ="'.$count.'" name="order_unit_cost[]" value="0.00"></td>
                                                           <td><input type="hidden" name="linetotal[]" id="linetotalfield'.$count.'"  value="'.$data[8].'">'.$data[8].'</td>
                                                           <td><input type="hidden" name="margin[]" id="marginfield'.$count.'" value="0.00">0.00</td>
                                                           <td><input type="hidden" name="margin_line[]" id="margin_linefield'.$count.'" value="'.$data[8].'">'.$data[8].'</td>
                                                          
                                                           <td class="status">
                                                                <select rno="'.$count.'" data-container="body" class="selectpicker costestimation" data-style="btn btn-warning btn-round" title="Choose Status" data-size="7" name="status[]" id="statusfield'.$count.'" required="true" onChange="CheckCostEstimation();">
                                                            ...
                                                            <td class="include">
                                                                <div class="checkbox">
                                                                    <label>
                                                                        <input type="checkbox" name="specification_check[]" rno ="'.$count.'" id="specificationcheck'.$count.'" onClick="changeSpecificationValue(this.id, this.getAttribute("rno"))">
                                                                    </label>
                                                                </div>
                                                                  <input type="hidden" name="include_in_specification[]" value="0" id="include_in_specification'.$count.'">
                                                            </td>
                                                            <td class="include">
                                                                <div class="checkbox">
                                                                    <label>
                                                                        <input type="checkbox" name="allowance_check[]" rno ="'.$count.'" id="allowance_check'.$count.'" onClick="changeAllowanceValue(this.id, this.getAttribute("rno"))" value="0">
                                                                    </label>
                                                                </div>
                                                                <input type="hidden" name="allowance[]" value="0" id="allowance'.$count.'"></td>
                                                           
                                                            <td class="comments">
                                                               <textarea name="comments[]" id="comments'.$count.'" class="form-control" style="width:200px;" placeholder="Enter comments here"></textarea>
                                                            </td>
                                                            <td class="boom_mobile"> 
                                                               <div class="checkbox">
                                                                    <label>
                        										        <input type="checkbox" name="boommobilecheck[]" rno ="'.$count.'"  id="boommobilecheck'.$count.'"  class="selected_items" onClick="changeBoomMobileValue(this.id,this.getAttribute("rno"))">
                                                                    </label>
                                                                </div>
                                                                <input type="hidden" name="hide_from_boom_mobile[]" value="0" id="hide_from_boom_mobile'.$count.'">
                        								
                        									</td>';
                        									
                                                                if(in_array(134, $this->session->userdata("permissions"))) {
                                                        
                        									$html .='<td class="add_task_to_schedule"> 
                                                               
                                                               <div class="checkbox">
                                                                    <label>
                        										        <input type="checkbox" name="addtasktoschedulecheck[]" rno ="'.$count.'"  id="addtasktoschedulecheck'.$count.'"  class="selected_items" onClick="changeAddTaskToScheduleValue(this.id,this.getAttribute("rno"))">
                                                                    </label>
                                                                </div>
                                                                <input type="hidden" name="add_task_to_schedule[]" value="0" id="add_task_to_schedule'.$count.'">
                        									</td>';
                        									 } 
                                                            $html .='<td class="text-right">
                                                               <a class="btn btn-simple btn-success btn-icon" id="iconlock'.$count.'" rno ="'.$count.'" onclick="changeLockStatus(this.getAttribute("rno"))"><i class="material-icons lock_icon_type lock_icon_type'.$count.'">lock_open</i></a>
                                                               <input type="hidden" name="is_line_locked[]" id="linelock'.$count.'" value="0">
