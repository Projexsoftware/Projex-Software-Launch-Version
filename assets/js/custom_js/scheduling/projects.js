$.validator.addMethod('uniqueTask', function(value) {
         ...
         var result = $.ajax({ 
			    ...
			    async:false, 
                url:base_url + 'scheduling/tasks/verify_task',
                type: 'post',
			    data:{name: value},
	}); 
...
$.validator.addMethod('uniqueProject', function(value) {
         ...
         var result = $.ajax({ 
			    ...
			    async:false, 
                url:base_url + 'scheduling/projects/verify_name',
                type: 'post',
			    data:{name: value},
	}); 
    ...
    $(document).ready(function() {
        ...
        setFormValidation('#ProjectLogForm');
 ...
 $('body').on('click','.edit_task',function(){
    ...
    if($("#TaskForm").valid()){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/update_project_item',
                data: data,
...
function get_stage_status(project_id, stage_id){
    ...
    $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/get_stage_status',
...
function get_project_items(){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/get_project_items',
                ...
                success:function(result){
                	...
                	$(".sortable_table tbody").sortable({
                		...
                		stop: function(event,ui) {
                                 ...
                                 var hasLinks = ui.item.attr("hasLinks");
                                 if(hasLinks == "false"){
                                    renumber_table('#'+table_id);
                                 }
                                 else{
                                    swal({
                                        title: "Dragging Error!",
                                        text: "You cannot drag this task, this task is dependent on another task or may be it has link to other tasks",
                                        timer: "",
                    ...
                    $("#accordionStages").sortable({
                       ...
                       opacity: 0.5,
                       stop : function(event, ui){
       var is_sortable_stage = "false";
       var table_id = "table_"+ui.item.attr("id");
       $("#"+table_id + " tr").each(function() {
           if($(this).attr("hasLinks") == "true"){
               is_sortable_stage = "true";
               return;
           }
       });
       if(is_sortable_stage == "false"){
           $("#sortable_stages").val(
            ...
            $("#accordionStages").sortable(
             ...
             $.ajax({
                        type:'POST',
                        url:base_url+'scheduling/projects/sort_stages',
                ...
                swal({
                        title: "Dragging Error!",
                        text: "You cannot drag this stage, this stage has a task that is dependent on another task or may be it has link to other tasks",
                        timer: "",
                        showConfirmButton: "yes",
                        type: "error"
                       }).catch(swal.noop);
...
$('.add_new_task').click(function(){
    ...
    if($("#TaskForm").valid()){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_project_item',
                data: data,
                success:function(result){
                    $("#TaskForm")[0].reset();
                    $(".selectpicker").val('default');
...
$(document).on('click','.remove_task',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_task',
...
var total_checklist_count = total_items - checklist_count;
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/update_task_checklist',
                ...
                success:function(result){
                    ...
                    if($("#workSpace").hasClass("ganttFullScreen")){
                        $( ".workSpace_container" ).load( base_url+"scheduling/projects/load_gantt_chart/"+project_id+"/true");
                    }
                    else{
...
$('body').on('click','.viewToggle',function(){
...
if ($('#'+selected_view).prop('checked')==true){
    ...
    $('#'+selected_view).attr('disabled', true);
    $('#'+other_view).attr('disabled', false);
}
else{
   var date_range = $("#daterange").val();
    date_range = date_range.split("/").join("-");
   ...
   $('#'+other_view).attr('disabled', true);
   $('#'+selected_view).attr('disabled', false);
   
}
if(selected_view == "calendar_view" && $('#calendar_view').prop('checked')==true){
    $(".project_items").html("");
}
else{
    get_project_items();
...
$(document).on('click','.add_new_checklist',function(){
	 ...
	 var item_id = $("#checklistForm"+rowno+" #checklist_item_id").val();			
         var result = $.ajax({ 
			    async:false, 
                url:base_url + 'scheduling/projects/verify_checklist',
                type: 'post',
    ...
    if(result.responseText == '0') return true; else return false;
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_checklist',
...
$(document).on('click','.add_new_note',function(){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_note',
...
$(document).on('click','.remove_note',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_note',
                   data: {id:id},
                   success:function(result){
                       swal({
                        title: "Note Deleted!",
...
$(document).on('click','.remove_checklist',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_checklist',
...
$(document).on('submit','.upload_task_file',function(e){
    ...
    $.ajax({
                ...
                url: base_url+'scheduling/projects/upload_file', 
                type: "POST",            
...
$(document).on('click','.remove_file',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_file',
                   data: {id:id,project_id:project_id,rowno:rowno},
                   success:function(result){
                       swal({
                        title: "File Deleted!",
...
$(document).on('submit','.upload_task_image',function(e){
    ...
    $.ajax({
                ...
                url: base_url+'scheduling/projects/upload_image', 
                type: "POST",            
                data: new FormData(this), 
                contentType: false,       
                cache: false,             
...
$(document).on('click','.remove_image',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_image',
                   data: {id:id,project_id:project_id,rowno:rowno},
                   success:function(result){
                       swal({
                        title: "Image Deleted!",
...
$(document).on('click','.add_new_reminder',function(){
    ...
    if(result.responseText == '0') return true; else return false;
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_reminder',
...
$(document).on('click','.remove_reminder',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_reminder',
                   data: {id:id},
                   success:function(result){
                       swal({
...
$(document).on('change','.add_from_template',function(){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_from_template',
...
$(document).on('change','.add_from_project',function(){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_from_project',
...
if(project_manager!="" || editor!="" || viewer!=""){
     ...
     $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_project_team',
...
$(document).on('click','.remove_project_team',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_project_team',
...
$(document).on('change','#message_id',function(){
        ...
        $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/get_reminder_message',
                   data: {id:id},
                   beforeSend: function(){
...
$(document).on('submit','.upload_project_document',function(e){
    ...
    $.ajax({
                ...
                url: base_url+'scheduling/projects/upload_document', 
                type: "POST",            
                data: new FormData(this), 
                contentType: false,       
                cache: false,             
...
$(document).on('click','.remove_document',function(){
          var id = $(this).attr("id");
          var project_id = $("#document_project_id").val();
          var document_parent_project_id = $("#document_parent_project_id").val();
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/remove_document',
                   data: {id:id,project_id:project_id,document_parent_project_id:document_parent_project_id},
                   success:function(result){
                       swal({
...
function renumber_table(tableID) {
	$(tableID + " tr").each(function() {
                ...
                $.ajax({
                    type:'POST',
                    url:base_url+'scheduling/projects/sort_item',
...
$(document).on('click','.invite_project_team',function(){
          ...
          swal({
                ...
                $.ajax({
                   type:'POST',
                   url:base_url+'scheduling/projects/invite_project_team',
                   data: {id:id,project_id:project_id},
                   success:function(result){
                       swal({
...
$.ajax({
                url: base_url+'scheduling/projects/set_privacy', 
                type: "POST",            
...
$(document).on('click','.add_new_user',function(){
          ...
          var result = $.ajax({ 
			    ...
			    async:false, 
                url:base_url + 'scheduling/users/verify_email',
                type: 'post',
			    data:{email: value},
	}); 
    ...
    if(result.responseText == '0') return true; else return false;
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_user',
                ...
                success:function(result){
                       ...
                       $(".leaders_users_container").load(base_url+"scheduling/projects/get_all_users/leaders");
                       $(".members_users_container").load(base_url+"scheduling/projects/get_all_users/members");
                       $(".guest_users_container").load(base_url+"scheduling/projects/get_all_users/guest");
                       $('.selectpicker').selectpicker('refresh');
                       demo.initFormExtendedDatetimepickers();
...
$(document).on('change','.assign_to_user',function(){
          ...
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/assign_task_to_user',
...
$(document).on('click','.btn-remove-new-log',function(){
   var rowno = $(this).attr("rowno"); 
   $(".row"+rowno).remove();
   $(".add-new-log").attr("disabled", false);
});

$(document).on('click','.add-new-log',function(){
          var count = $(".project-logs-container .new_log_row").length;
          $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/add_new_log',
                data: {count:count+1},
                success:function(row){
                       $(".add-new-log").attr("disabled", true);
                       $("#ProjectLogForm").attr("rowno", count+1);
                       $(".project-logs-container").prepend(row);
                }
             }); 
});

$(document).on('submit','.project_log_form',function(e){
          e.preventDefault();
          if($("#ProjectLogForm").valid()){
            var rowno = $(this).attr("rowno"); // Get the `rowno` attribute
            var project_id = $("#original_project_id").val();   
            var task_file = $('#log_image'+rowno).val();
            ...
            $.ajax({
                xhr: function() {
                    ...
                    xhr.upload.addEventListener("progress", function(evt) {
                      if (evt.lengthComputable) {
                        var percentComplete = evt.loaded / evt.total;
                        percentComplete = parseInt(percentComplete * 100);
                        $("#progressLogBar"+rowno).text(percentComplete+"%");
                        $("#progressLogBar"+rowno).css("width", percentComplete+"%");
                          }
                        }, false);
                    
                        return xhr;
                },
                url: base_url+'scheduling/projects/new_log_process', 
                type: "POST",            
                data: formData, 
                contentType: false,       
                cache: false,             
                processData:false,
                beforeSend: function(){
                    $("#btn-add-new-log"+rowno).text("Please Wait Project Log is Saving....");
                    $("#btn-add-new-log"+rowno).attr("disabled", true);
                },
                success: function(result){
                    $("#btn-add-new-log"+rowno).text("Add");
                    $("#btn-add-new-log"+rowno).attr("disabled", "false");
                    $(".add-new-log").attr("disabled", false);
                    $("#progressLogBar"+rowno).text("0%");
                    $("#progressLogBar"+rowno).css("width", "0%");
                    swal({
                        title: "New Project Log Created!",
                        text: "",
                        timer: 2000,
                        showConfirmButton: false,
                        type: "success"
                       }).catch(swal.noop);
                       $(".project-logs-container").html(result);
                }
     });
            }
          else{
              $("#ProjectLogForm").validate();
          }
});

function get_project_logs(){
            var project_id = $("#original_project_id").val();   
            $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/project_logs',
                data: {project_id:project_id},
                success:function(result){
                       $(".project-logs-container").html(result);
                }
...
function get_project_documents(){
            ...
            $.ajax({
                type:'POST',
                url:base_url+'scheduling/projects/project_documents',
                data: {project_id:project_id},
                success:function(result){
                      $("#document_container").html(result);
                }
